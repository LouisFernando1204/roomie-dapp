...
const CourtDetail: React.FC<CourtDetailProps> = ({ walletProvider }) => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchSpecificCase = async () => {
      ...
      try {
        ...
        if (res) {
          const caseDetailData = await caseDetail(res.id);
  ...
  const voteCase = async (caseId: string, side: number) => {
    setLoading(true);
    try {
      const tx = await voteOnCase(caseId, side, walletProvider);
        ...
        <div className="w-5/6 h-full flex flex-col justify-center items-center space-y-4">
          ...
          <div className="w-full flex flex-col justify-start items-start space-y-2">
            <span className="font-normal text-gray-900 text-base">
              Customer Voted by: {courtCase?.totalCustomerVote} people
            </span>
            <span className="font-normal text-gray-900 text-base">
              Accommodation Voted by: {courtCase?.totalAccommodationVote} people
            </span>
          </div>
          {/* CUSTOMER CASE */}
            ...
            >
              <div className="mb-12 lg:mb-0 lg:grid lg:grid-cols-2 lg:gap-8 xl:gap-16">
                ...
                <div className="mt-4 lg:mt-0">
                  ...
                  courtCase.userCases.length > 0 ? (
                    <button
                      onClick={() => {
                        handleVote(courtCase.id, 1);
                      }}
                      type="button"
                      className="w-full mt-4 text-white bg-primary hover:bg-darkYellow focus:outline-none font-medium rounded-lg text-base px-4 py-2 text-center inline-flex justify-center items-center"
                    >
                      <svg
                        className="w-4 h-4 me-2 -ms-1 text-base text-white"
                        aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
                        height="24"
                    ...
                    <button
                      disabled
                      type="button"
                      className="w-full mt-4 text-white bg-gray-300 hover:bg-gray-400 focus:outline-none font-medium rounded-lg text-base px-4 py-2 text-center inline-flex justify-center items-center"
                    >
                      <svg
                        className="w-4 h-4 me-2 -ms-1 text-base text-white"
                        aria-hidden="true"
              ...
              <div className="mb-12 lg:mb-0 lg:grid lg:grid-cols-2 lg:gap-8 xl:gap-16">
                <div className="shrink-0 max-w-md lg:max-w-sm mx-auto flex justify-center items-center">
                  {accCase.accommodationEvidence ? (
                    <img
                      className="w-full"
                      src={accCase.accommodationEvidence}
                      alt="Accommodation Case"
                    />
                  ) : (
                    <div className="flex justify-center items-center w-full h-[200px]">
                      <svg
