...
const Court = () => {
      ...
      if (res) {
        const structuredCourtCases = await Promise.all(
          res.map(async (courtCase: Case) => {
            const caseDetailData = await caseDetail(courtCase.id);
            return {
              ...courtCase,
  ...
  return (
    <div className="px-4 md:px-12">
      <div className="w-full h-full flex justify-center items-center mb-16">
                ...
                <div className="p-4">
                  <div className="space-y-2 flex flex-col">
                    ...
                    <span className="text-base font-normal text-gray-500">
                      Booking Id: {courtCase.bookingId}
                    </span>
                    <span className="text-base font-normal text-gray-500">
                      Customer Voted by: {courtCase.totalCustomerVote} people
                    </span>
                    <span className="text-base font-normal text-gray-500">
                      Accommodation Voted by: {courtCase.totalAccommodationVote}{" "}
                      people
                    </span>
                    <span className="text-base font-normal text-gray-500">
                      {`Created at: ${format(
                  ...
                  <Link to={`/court/${courtCase.id}`}>
                    <button
                      type="button"
                      className="mt-6 w-full rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:outline-none"
                    >
                      View details
                    </button>
                  </Link>
